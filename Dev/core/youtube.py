...
class YouTube:
    ...
    def _make_opts(self, extractor_args: dict) -> dict:
        return {
    ...
    def _try_download(self, url: str, video_id: str, video: bool, extractor_args: dict) -> Optional[str]:
        opts = self._make_opts(extractor_args)
    ...
    async def download(self, video_id: str, video: bool = False) -> Optional[str]:
        ...
        for client_args in CLIENTS:
            result = await asyncio.to_thread(
                self._try_download, url, video_id, video, client_args
            )
            if result:
                return result
            await asyncio.sleep(0.5)

        logger.error("All clients failed for video: %s", video_id)
        return None
