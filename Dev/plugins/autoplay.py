...
# {chat_id: {"playlist": [...video_dicts], "index": 0, "seed_id": ""}}
...
async def get_next_autoplay_track(chat_id: int, last: dict) -> dict | None:
    ...
    # If we have a playlist loaded for this seed, use next track from it
    if state.get("seed_id") == last_id and state.get("playlist"):
        idx = state.get("index", 0)
        playlist = state["playlist"]
        if idx < len(playlist):
            track = playlist[idx]
            autoplay_state[chat_id]["index"] = idx + 1
            return track
        else:
            # Playlist exhausted, fetch new one from last track
            pass
    ...
    autoplay_state[chat_id] = {
        "seed_id": last_id,
        "playlist": playlist,
        "index": 1,  # 0 already being returned now
    }
...
async def autoplay_cmd(_, m: types.Message):
    ...
    if args in ("on", "enable"):
        ...
        autoplay_state.pop(chat_id, None)
